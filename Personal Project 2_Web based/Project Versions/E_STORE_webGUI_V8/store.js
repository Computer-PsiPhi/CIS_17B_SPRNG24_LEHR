...
class Controller {
    ...
    init() {
        ...
        document.cookie.split(";").forEach((c) => {
            document.cookie = c.trim().startsWith("listCart=") ?
                c.replace(/=.*/, "=;expires=" + new Date().toUTCString() + ";path=/") :
                c;
        });

        // Add a unique query parameter to the fetch URL
        const url = `products.json?v=${new Date().getTime()}`;
        fetch(url)
            .then(response => {
                if (!response.ok) {
