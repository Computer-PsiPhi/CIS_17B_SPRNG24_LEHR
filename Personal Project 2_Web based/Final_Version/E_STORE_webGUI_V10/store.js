class Product {
    constructor(id, name, price, image) {
        this.id = id;
        this.name = name;
        this.price = parseFloat(price); // Ensuring price is a double
        this.image = image;
    }
}
...
class View {
    ...
    displayCart(cart) {
        ...
        cart.forEach(item => {
            ...
            let formattedPrice = (item.product.price * item.quantity).toFixed(2);
            newItem.innerHTML = `
                <div class="image">
...
class Controller {
    ...
    init() {
        fetch('products.json')
            ...
            .then(data => {
                ...
                this.setProductsCookie(); // Set cookie with products data
            })
            .catch(error => {
    ...
    setProductsCookie() {
        const productsData = JSON.stringify(this.store.products);
        document.cookie = `products=${encodeURIComponent(productsData)}; path=/;`;
    }
